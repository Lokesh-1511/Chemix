...
import '../styles/PeriodicTableNew.css';

const PeriodicTable = () => {
  const [selected, setSelected] = useState(null);

  // Build a 2D map: period x group -> element (exclude f-block from main grid)
      ...
      <div className="pt-container">
        ...
        <div className="pt-panel">
          <div className="pt-grid" style={{
            ...
            {Array.from({ length: tableConfig.periods }).map((_, r) => (
              Array.from({ length: tableConfig.groups }).map((_, c) => {
                const period = r + 1;
                const group = c + 1;
                const el = gridMap.get(`${period}-${group}`);
                return (
                  <div key={`${period}-${group}`} className={`pt-cell ${el ? 'has-el' : 'empty'}`}>
                    {el && (
                      <button
